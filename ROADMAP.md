...
| Hygiene: tag/release v0.3.0 (and note v0.2.0 was never tagged), GitHub description and topics, companion-repo link, `dev/validate.sh` hardening | `docs/research/quick-wins.md` #1–#7 | repo settings, `dev/validate.sh`, `.github/workflows/ci.yml` | S | validate.sh green; CI pins `@anthropic-ai/claude-code` | v1 |
...
1. Approve or reject the v0.6 planner as within audit-only scope (generated text, never applied).
2. Approve or reject opening SPEC v2 for v1.0 IMPLEMENT and for enforcement hooks.
3. Whether to back-tag v0.2.0 or move straight to v0.3.0.
4. Whether the dbt static audit belongs in this plugin or in a sibling.
