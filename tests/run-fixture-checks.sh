...
step "db-access-audit lakeformation pack (v0.11, partial by design): recorded AWS CLI JSON -> module verdicts"
LF="tests/fixtures/lakeformation"; LFR="arn:aws:iam::111122223333:role/ai-agent"
python3 skills/db-access-audit/scripts/eval_grants.py --dialect lakeformation --recorded "$LF" \
  --role "$LFR" --principal-confirmed --ignore-dir "$LF" --emit-json "$TMP/out.json"
assert "DB-01 CRITICAL: INSERT on retail.orders (direct) and ALL via IAM_ALLOWED_PRINCIPALS on legacy_profiles; write-only labels" \
  '.findings | any(.check_id == "DB-01" and .severity == "CRITICAL" and (.evidence | test("retail.orders:INSERT \\(direct\\)")) and (.evidence | test("retail.legacy_profiles:ALL \\(via IAM_ALLOWED_PRINCIPALS\\)")) and (.evidence | test("SELECT") | not))'
assert "DB-02 HIGH: customers reaches 4 columns (exclusion list), orders and legacy_profiles all columns" \
  '.findings | any(.check_id == "DB-02" and .severity == "HIGH" and (.evidence | test("retail.customers \\(4 column\\(s\\)\\)")) and (.evidence | test("retail.legacy_profiles \\(all columns\\)")) and (.evidence | test("retail.orders \\(all columns\\)")))'
assert "DB-03: ssn/card_number excluded by the column wildcard so absent; legacy date_of_birth HIGH; email/full_name/phone MEDIUM" \
  '(.findings | any(.check_id == "DB-03" and .severity == "HIGH" and (.evidence | test("retail.legacy_profiles.date_of_birth")) and (.evidence | test("retail.customers.ssn") | not) and (.evidence | test("card_number") | not))) and (.findings | any(.check_id == "DB-03" and .severity == "MEDIUM" and (.evidence | test("retail.customers.email")) and (.evidence | test("retail.legacy_profiles.phone"))))'
assert "DB-08 HIGH: legacy_profiles read with no column list or granted cells filter; customers has an exclusion list; the existing cells filter is not granted" \
  '.findings | any(.check_id == "DB-08" and .severity == "HIGH" and (.evidence | test("granted cells filter: retail.legacy_profiles\\.")) and (.evidence | test("Cells filters exist on retail.customers but are not granted")))'
assert "DB-04 MEDIUM: no curated/masked layer among the Glue tables" '.findings | any(.check_id == "DB-04" and .severity == "MEDIUM")'
assert "DB-07 HIGH (settings): 'Use only IAM access control' defaults grant IAMAllowedPrincipals on new tables" \
  '.findings | any(.check_id == "DB-07" and .severity == "HIGH" and (.evidence | test("Use only IAM access control")))'
assert "DB-07 HIGH (everyone/hybrid): IAM_ALLOWED_PRINCIPALS grant; legacy_profiles under the hybrid location with the principal not opted in" \
  '.findings | any(.check_id == "DB-07" and (.evidence | test("Grants to everyone")) and (.evidence | test("hybrid access mode")) and (.evidence | test("retail.legacy_profiles at s3://acme-lake/legacy/profiles/ \\(principal not opted in")))'
assert "DB-05 INFO/probable (one multi-region trail) + DB-09 MEDIUM/probable: no query text, S3 data events not assessed, Redshift federated exception" \
  '(.findings | any(.check_id == "DB-05" and .severity == "INFO" and (.evidence | test("1 trail\\(s\\) \\(1 multi-region")) and (.evidence | test("1 data lake admin")))) and (.findings | any(.check_id == "DB-09" and .confidence == "probable" and (.evidence | test("Query text is not")) and (.evidence | test("S3 data events, whose enablement was not captured")) and (.evidence | test("Redshift federated catalogs"))))'
assert "DB-ID-01 HIGH: AmazonS3ReadOnlyAccess bypasses Lake Formation at unregistered/hybrid locations; IAM role (no USER complaint)" \
  '.findings | any(.check_id == "DB-ID-01" and .severity == "HIGH" and (.evidence | test("AmazonS3ReadOnlyAccess")) and (.evidence | test("IAM USER") | not))'
assert "DB-10 HIGH: DATA_LOCATION_ACCESS on s3://acme-lake/raw" '.findings | any(.check_id == "DB-10" and .severity == "HIGH" and (.evidence | test("acme-lake/raw")))'
assert "DB-06: customer-managed policy judged by name only + inline/group/boundary policies never expanded; explicit-grants precondition" \
  '(.unknowns | any(.reason | test("acme-agent-glue-read were judged by name only") and test("inline policies, group memberships"))) and (.unknowns | any(.reason | test("explicitly granted permissions only")))'
assert "plan_inputs: principal_kind role, everyone_grants, hybrid_locations + hybrid_iam_governed, no unresolved grants, partial_by_design, planner unsupported" \
  '(.plan_inputs.principal_kind == "role") and (.plan_inputs.everyone_grants | length == 1) and (.plan_inputs.hybrid_locations == ["arn:aws:s3:::acme-lake/legacy"]) and (.plan_inputs.hybrid_iam_governed | length == 1) and (.plan_inputs.unresolved_grants == []) and (.plan_inputs.partial_by_design == true) and (.plan_inputs.planner_supported == false)'
assert "every lakeformation finding carries citations and a fingerprint" '[.findings[] | (.citations | length > 0) and (.fingerprint | length > 0)] | all'
# an IAM user with AdministratorAccess as the agent -> CRITICAL; data lake admin -> CRITICAL DB-07
LFU="$TMP/lf-user"; mkdir -p "$LFU"; cp "$LF"/*.json "$LFU"/
echo '{"AttachedPolicies": [{"PolicyName": "AdministratorAccess", "PolicyArn": "arn:aws:iam::aws:policy/AdministratorAccess"}]}' > "$LFU/iam_policies.json"
jq '.DataLakeSettings.DataLakeAdmins += [{"DataLakePrincipalIdentifier": "arn:aws:iam::111122223333:user/ai-agent-user"}]' "$LF/settings.json" > "$LFU/settings.json"
python3 skills/db-access-audit/scripts/eval_grants.py --dialect lakeformation --recorded "$LFU" --role "arn:aws:iam::111122223333:user/ai-agent-user" --principal-confirmed --emit-json "$TMP/out-lfu.json"
if jq -e '(.findings | any(.check_id == "DB-ID-01" and .severity == "CRITICAL" and (.evidence | test("IAM USER")) and (.evidence | test("AdministratorAccess")))) and (.findings | any(.check_id == "DB-07" and .severity == "CRITICAL" and (.evidence | test("Data lake administrator"))))' "$TMP/out-lfu.json" >/dev/null; then echo "OK: IAM user + AdministratorAccess + data lake admin -> CRITICAL identity and settings findings"; else echo "FAIL: LF admin principal handling"; fail=1; fi
# missing tables.json -> no DB-03/DB-08 fabricated, DB-06 present; malformed permissions -> DB-06
LFM="$TMP/lf-missing"; mkdir -p "$LFM"; cp "$LF"/*.json "$LFM"/; rm "$LFM/tables.json"; echo '{"nope": []}' > "$LFM/permissions.json"
python3 skills/db-access-audit/scripts/eval_grants.py --dialect lakeformation --recorded "$LFM" --role "$LFR" --principal-confirmed --emit-json "$TMP/out-lfm.json"
if jq -e '([.findings[] | select(.check_id == "DB-03" or .check_id == "DB-08" or .check_id == "DB-01" or .check_id == "DB-02")] | length == 0) and (.unknowns | any(.reason | test("tables.json. missing"))) and (.unknowns | any(.reason | test("permissions.json has no PrincipalResourcePermissions")))' "$TMP/out-lfm.json" >/dev/null; then echo "OK: missing Glue schemas and malformed permissions -> DB-06s, nothing fabricated"; else echo "FAIL: LF fail-closed handling"; fail=1; fi
# no registered locations -> DB-06 that IAM alone governs
LFN="$TMP/lf-noreg"; mkdir -p "$LFN"; cp "$LF"/*.json "$LFN"/; echo '{"ResourceInfoList": []}' > "$LFN/resources.json"
python3 skills/db-access-audit/scripts/eval_grants.py --dialect lakeformation --recorded "$LFN" --role "$LFR" --principal-confirmed --emit-json "$TMP/out-lfn.json"
if jq -e '.unknowns | any(.reason | test("no S3 locations are registered"))' "$TMP/out-lfn.json" >/dev/null; then echo "OK: no registered locations -> DB-06 (IAM alone governs)"; else echo "FAIL: unregistered handling"; fail=1; fi
# gate fixes (v0.11 Codex gate): LF-tag grants -> DB-06; filter granted with SELECT narrows columns; SUPER_USER on the catalog; malformed settings; missing cloudtrail / opt-ins
LFT="$TMP/lf-tags"; mkdir -p "$LFT"; cp "$LF"/*.json "$LFT"/
jq '.PrincipalResourcePermissions += [{"Principal":{"DataLakePrincipalIdentifier":"'"$LFR"'"},"Resource":{"LFTagPolicy":{"CatalogId":"111122223333","ResourceType":"TABLE","Expression":[{"TagKey":"tier","TagValues":["gold"]}]}},"Permissions":["SELECT"],"PermissionsWithGrantOption":[]},{"Principal":{"DataLakePrincipalIdentifier":"'"$LFR"'"},"Resource":{"DataCellsFilter":{"TableCatalogId":"111122223333","DatabaseName":"retail","TableName":"customers","Name":"customers_no_pii"}},"Permissions":["SELECT"],"PermissionsWithGrantOption":[]}] | .PrincipalResourcePermissions |= map(select((.Resource.TableWithColumns.Name // "") != "customers"))' "$LF/permissions.json" > "$LFT/permissions.json"
python3 skills/db-access-audit/scripts/eval_grants.py --dialect lakeformation --recorded "$LFT" --role "$LFR" --principal-confirmed --emit-json "$TMP/out-lft.json"
if jq -e '(.unknowns | any(.reason | test("LF-tag policy TABLE:") and test("NOT added to DB-02/03/08"))) and (.findings | any(.check_id == "DB-02" and (.evidence | test("retail.customers \\(2 column\\(s\\)\\)")))) and ([.findings[] | select(.check_id == "DB-03") | .evidence | test("retail.customers")] | any | not) and (.findings | any(.check_id == "DB-08" and (.evidence | test("retail.customers") | not)))' "$TMP/out-lft.json" >/dev/null; then echo "OK: LF-tag grant -> DB-06 (not counted); cells filter granted with SELECT narrows customers to 2 columns and protects it"; else echo "FAIL: LF-tag / cells-filter handling"; fail=1; fi
LFS="$TMP/lf-super"; mkdir -p "$LFS"; cp "$LF"/*.json "$LFS"/
jq '.PrincipalResourcePermissions += [{"Principal":{"DataLakePrincipalIdentifier":"'"$LFR"'"},"Resource":{"Catalog":{}},"Permissions":["SUPER_USER"],"PermissionsWithGrantOption":[]}]' "$LF/permissions.json" > "$LFS/permissions.json"
echo '{"AttachedPolicies": [{"PolicyName": "AWSGlueConsoleFullAccess", "PolicyArn": "arn:aws:iam::aws:policy/AWSGlueConsoleFullAccess"}]}' > "$LFS/iam_policies.json"
python3 skills/db-access-audit/scripts/eval_grants.py --dialect lakeformation --recorded "$LFS" --role "$LFR" --principal-confirmed --emit-json "$TMP/out-lfs.json"
if jq -e '(.findings | any(.check_id == "DB-01" and (.evidence | test("catalog:SUPER_USER")))) and (.findings | any(.check_id == "DB-02" and (.evidence | test("marketing.campaigns \\(all columns\\)")) and (.evidence | test("SUPER_USER on the catalog")))) and (.findings | any(.check_id == "DB-03" and (.evidence | test("retail.customers.ssn")))) and ([.findings[] | select(.check_id == "DB-ID-01")] | length == 0) and (.unknowns | any(.reason | test("AWSGlueConsoleFullAccess were judged by name only")))' "$TMP/out-lfs.json" >/dev/null; then echo "OK: catalog SUPER_USER reaches every table incl. the second database; AWSGlueConsoleFullAccess is by-name DB-06, not CRITICAL"; else echo "FAIL: SUPER_USER / policy classifier"; fail=1; fi
LFB="$TMP/lf-bad"; mkdir -p "$LFB"; cp "$LF"/*.json "$LFB"/; echo '{"DataLakeSettings": []}' > "$LFB/settings.json"; rm "$LFB/cloudtrail.json" "$LFB/opt_ins.json"; echo '{"AttachedPolicies": []}' > "$LFB/iam_policies.json"
python3 skills/db-access-audit/scripts/eval_grants.py --dialect lakeformation --recorded "$LFB" --role "$LFR" --principal-confirmed --emit-json "$TMP/out-lfb.json"
if jq -e '([.findings[] | select(.check_id == "DB-05" or .check_id == "DB-09")] | length == 0) and (.unknowns | any(.reason | test("no well-formed DataLakeSettings"))) and (.unknowns | any(.reason | test("cloudtrail.json was not captured"))) and (.unknowns | any(.reason | test("opt_ins.json was not captured"))) and ([.findings[] | select(.check_id == "DB-07") | .evidence | test("hybrid")] | any | not) and (.unknowns | any(.reason | test("inline policies, group memberships")))' "$TMP/out-lfb.json" >/dev/null; then echo "OK: malformed settings, missing cloudtrail and opt-ins -> DB-06s; no DB-05/09, no hybrid DB-07 claim; empty AttachedPolicies still DB-06"; else echo "FAIL: LF settings/cloudtrail/opt-in fail-closed"; fail=1; fi
LFO="$TMP/lf-optin"; mkdir -p "$LFO"; cp "$LF"/*.json "$LFO"/
jq '.LakeFormationOptInsInfoList[0].Principal.DataLakePrincipalIdentifier = "'"$LFR"'"' "$LF/opt_ins.json" > "$LFO/opt_ins.json"
python3 skills/db-access-audit/scripts/eval_grants.py --dialect lakeformation --recorded "$LFO" --role "$LFR" --principal-confirmed --emit-json "$TMP/out-lfo.json"
